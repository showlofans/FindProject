...
import com.stufinish.findproject.activity.MainApplication;
...
public class ConfigCacheUtil {
    ...
    public static String getUrlCache(String url, ConfigCacheModel model) {
        ...
        if(file.exists() && file.isFile()) {
            ...
            // 1。如果系统时间是不正确的
            // 2。当网络是无效的,你只能读缓存
            if(NetUtils.isConnected(MainApplication.getInstance())) {
                if(expiredTime < 0) {
                    return null;
                }
            ...
            try {
                result=File2StringUtil.readTextFile(file);
            } catch(IOException e) {
                e.printStackTrace();
            }
        }
    ...
    public static void setUrlCache(String data, String url) {
        ...
        File file=new File(url+"/cache");
        try {
            // 创建缓存数据到磁盘，就是创建文件
            File2StringUtil.writeTextFile(file, data);
        } catch(IOException e) {
