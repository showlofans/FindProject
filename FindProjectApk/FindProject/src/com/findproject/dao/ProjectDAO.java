...
public class ProjectDAO {
	...
	public ProjectBean getProject(int proj_id) {
		...
		try {
			...
			String sql_proj = "select * from project where proj_id=" + proj_id;
			pstm = conn.prepareStatement(sql_proj);
	...
	public ArrayList<ProjectBean> getAllItems(int currentPage, int pageSize,
		...
		ArrayList<ProjectBean> list = new ArrayList<ProjectBean>();
		StringBuffer sql = new StringBuffer();
		int count = getCount(email, type);
		if (count < pageSize) {// 判断首页是否大于总数
			sql.append("select * from project where where proj_mail != '");
			sql.append(email);
			if (type != "项目") {
